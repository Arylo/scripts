...
const InputGroup = lazy(() =>
  import('../ui/input-group').then((m) => ({ default: m.InputGroup })),
)
...
export default function CodeAddressButton({ codeValue, buttonProps = {} }: Props) {
  ...
  return (
    <Popover>
      <PopoverTrigger render={<Button variant="outline" size="sm" {...buttonProps} className={cc(["cursor-pointer", buttonProps.className])} />}>
        提取地址
      </PopoverTrigger>
