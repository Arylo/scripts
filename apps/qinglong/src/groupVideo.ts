...
const fileListMap = FILE_EXT.reduce<string[]>((acc, ext) => {
  return [...acc, ...ls(path.resolve(rootPath, `./*${ext}`))]
}, []).map((p) => {
  const stat = fs.statSync(p)
  ...
  return {
    ...restObj,
    tags: TAG_FILTER_FNS.map((fn) => fn(baseObj))
      .filter(Boolean)
      .map((tag) => tag?.toUpperCase()) as string[],
  }
})
