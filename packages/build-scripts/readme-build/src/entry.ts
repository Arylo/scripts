...
import parseScriptInfo from '@scripts/build-utils'
...
const CONSTANT = {
  GENERAL: {
    HEADER: genTemplate((utils) => {
      ...
      utils.hyperlink(
        MdTools.image(
          'https://img.shields.io/github/license/arylo/scripts.svg?style=flat-square&logo=github&cacheSecond=7200',
          'GitHub license',
        ),
  ...
  MONKEY: {
    ...
    FOOTER: genTemplate((utils) => {
      utils.anchor(
        'pass',
        'https://img.shields.io/badge/-pass-green.svg?&logoColor=000&style=for-the-badge&cacheSeconds=7200',
      )
      utils.anchor(
        'unknown',
        'https://img.shields.io/badge/-unknown-silver.svg?&logoColor=000&style=for-the-badge&cacheSeconds=7200',
      )
    }),
  },
  QINGLONG: {
...
function parseMonkeyMd(folderPath: string) {
  ...
  content = readTemplate(content, (utils) => {
    ...
    if (installContent.length) {
      utils.modify((currentContent) => {
        ...
        const descriptionIndex = contentList.findIndex(
          (v) => v.trimEnd() === MdTools.h2('Description'),
        )
    ...
    utils.modify((currentContent) => {
      ...
      contentList.splice(
        descriptionIndex + 1,
        0,
        ...[MdTools.enter(), MdTools.imageByKey(anchorKey, 'GitHub last commit'), MdTools.enter()],
      )
    ...
    utils.modify((currentContent) => {
      ...
      contentList.splice(
        ...
        ...[
          ...
          MdTools.hyperlink(
            'Top',
            `#${lodash.kebabCase(CONSTANT.MONKEY.HEADER.split(/\n/)[0].replace(/^[\s#]+/, ''))}`,
          ),
          MdTools.enter(),
        ],
      )
  ...
  const isDeprecated = buildFS
    .readJSONFileSync(infos[0].bannerFilePath)
    ?.description?.startsWith('(Deprecated)')
  if (isDeprecated) {
    content = content
      .split(/\n/)
      .map((line) =>
        isDeprecated && /^[\w#]/.test(line) ? line.replace(/^(#{1,5}\s+)?(.+)$/, '$1~~$2~~') : line,
      )
      .join('\n')
  }
...
const parseMonkeys = () => {
  ...
  const tableContent = genTemplate((utils) => {
    ...
    monkeyReadmePaths.forEach((p) => {
      ...
      let infos: string[] = [
        ...
        MdTools.hyperlink(
          'Jump',
          `#${lodash.kebabCase(buildFS.readFileSync(path.resolve(p)).match(/^# (.+?)\n/)?.[1])}`,
        ),
      ]
  ...
  return [tableContent, scriptMd].join('\n\n')
}
