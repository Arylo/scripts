...
import { ScriptInfo } from '@scripts/build-utils'
    ...
    if (isCI) {
      ...
      if (githubRawPrefix) {
        ...
        try {
          ...
          await download(latestDeployUrl, path.dirname(tmpJsonPath), {
            filename: path.basename(tmpJsonPath),
          })
          ...
          logger.info(
            ...
            `[current:${contentHash}]`,
            `${isDiffContent ? '!' : '='}=`,
            `[remote:${downloadDeployJson.contentHash}]`,
          )
          logger.info(
            'BannerHash: ',
            `[current:${bannerHash}]`,
            `${isDiffBanner ? '!' : '='}=`,
            `[remote:${downloadDeployJson.bannerHash}]`,
          )
...
export const buildScript = (filepath: string, extraConfig: esbuild.BuildOptions = {}) => {
  ...
  return esbuild.build({
    ...
    ...(isCI
      ? {
          plugins: [CSSMinifyTextPlugin(), HTMLMinifyTextPlugin()],
        }
      : {
          loader: {
            '.css': 'text',
